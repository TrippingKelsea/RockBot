...
fn render_header(frame: &mut Frame, area: Rect, state: &AppState) {
    ...
    let title = Line::from(vec![
        Span::styled("🚀 RockBot Dashboard", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::raw(" "),
        status_indicator,
    ]);
...
fn render_agents_table(frame: &mut Frame, area: Rect, state: &AppState) {
    ...
    if state.agents.is_empty() {
        let content = Paragraph::new(vec![
            ...
            Line::from(""),
            Line::from(Span::styled("Add agents in rockbot.toml", Style::default().fg(Color::DarkGray))),
        ])
        .block(block)
        .alignment(Alignment::Center);
