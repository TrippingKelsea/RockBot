...
impl A2ADispatcher {
    ...
    async fn handle_tasks_get(
        ...
        let Some(task_id) = params.get("taskId").or_else(|| params.get("task_id")).and_then(|v| v.as_str()) else {
        ...
        match self.task_store.get_task(task_id).await {
            ...
            None => JsonRpcResponse::error(id, TASK_NOT_FOUND, format!("Task '{task_id}' not found")),
        }
    }
    ...
    async fn handle_tasks_cancel(
        ...
        if self.task_store.update_status(task_id, TaskStatus::Canceled).await {
            JsonRpcResponse::success(
                id,
                serde_json::json!({
                    "taskId": task_id,
...
mod tests {
    ...
    fn test_part_serialization() {
        ...
        let data = Part::Data { data: serde_json::json!({"key": "value"}) };
