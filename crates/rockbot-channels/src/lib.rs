//! Communication channel traits and registry for RockBot.
//!
//! This crate defines the [`Channel`] trait and [`ChannelRegistry`] for dynamic
//! provider registration. Concrete channel implementations live in per-channel
//! crates (`rockbot-channels-discord`, `rockbot-channels-telegram`, etc.).
...
pub trait Channel: Send + Sync {
    ...
    /// Credential schema describing what this channel needs to authenticate.
...
/// Channel registry — collects channels and their credential schemas.
///
/// Mirrors `LlmProviderRegistry` so that channels self-register their
/// credential requirements and the gateway can query them dynamically.
...
impl ChannelRegistry {
    ...
    /// Collect credential schemas from all registered channels
