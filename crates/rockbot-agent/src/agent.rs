...
- Repeating the same failed tool call without changing the arguments or approach."#
    ...
    async fn run_reflection(
        ...
        let assistant = crate::from_llm_message(assistant_msg, session_id, &self.config.id)?;
    ...
    async fn execute_workflow(
        ...
        workflow: &rockbot_config::WorkflowDefinition,
        progress_tx: &Option<ProgressSender>,
...
mod tests {
    ...
    fn test_agent_response_serialization() {
        let response = AgentResponse {
            message: rockbot_config::Message::text("Hello"),
