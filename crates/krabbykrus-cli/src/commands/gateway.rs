    ...
    let agent_factory: krabbykrus_core::gateway::AgentFactory = Arc::new(move |agent_config: AgentInstance| {
        ...
        Box::pin(async move {
            ...
            let llm_provider = llm.get_provider_for_model(model).await
                .map_err(|e| krabbykrus_core::error::GatewayError::InvalidRequest {
                    message: e.to_string(),
                })?;
...
async fn start_service() -> Result<()> {
    ...
    let status = Command::new("systemctl")
        .args(["--user", "start", "krabbykrus-gateway"])
        .status();
    ...
    match status {
        ...
        _ => {
            ...
            if status.success() {
                ...
                anyhow::bail!("Failed to start gateway service. Is it installed? Run 'krabbykrus gateway install'")
            }
        }
    }
}
...
async fn stop_service() -> Result<()> {
    // Try user service first, then system
    let status = Command::new("systemctl")
        .args(["--user", "stop", "krabbykrus-gateway"])
        .status();
...
async fn restart_service() -> Result<()> {
    // Try user service first, then system
    let status = Command::new("systemctl")
        .args(["--user", "restart", "krabbykrus-gateway"])
        .status();
    ...
    match status {
        ...
        _ => {
            // Try system service
            let status = Command::new("systemctl")
                .args(["restart", "krabbykrus-gateway"])
                .status()?;
...
async fn show_status() -> Result<()> {
    ...
    let _ = Command::new("systemctl")
        .args(["--user", "status", "krabbykrus-gateway", "--no-pager"])
        .status();
    
    println!("\n=== System Service ===");
    let _ = Command::new("systemctl")
        .args(["status", "krabbykrus-gateway", "--no-pager"])
        .status();
...
async fn install_service(system: bool, name: &str, config_path: &PathBuf) -> Result<()> {
    ...
    let service_content = if system {
        // System service
        format!(r#"[Unit]
Description=Krabbykrus Gateway Server
...
"#,
    ...
    println!();
    println!("Or use: krabbykrus gateway start/stop/status/logs");
