...
use krabbykrus_credentials::{CredentialManager, UnlockMethod};
...
pub async fn unlock_vault_for_gateway(
    ...
    let unlock_method = match manager.get_unlock_method().await {
        ...
        None => {
            ...
            return Err(anyhow!(
                "Vault not initialized. Run 'krabbykrus credentials init' first."
            ));
        }
    };
    ...
    match unlock_method {
        ...
        UnlockMethod::Password { .. } => {
            ...
            let password = if let Some(env_var) = password_env {
                ...
                // Default env var
                std::env::var("KRABBYKRUS_VAULT_PASSWORD").ok()
            };
            ...
            if let Some(password) = password {
                ...
                return Err(anyhow!(
                    "Vault requires password but running non-interactively. \
                     Set KRABBYKRUS_VAULT_PASSWORD or use --interactive"
                ));
            }
        }
    ...
    if credentials.is_empty() {
        warn!(
            "No LLM API keys found in vault or environment. \
             Add credentials with 'krabbykrus credentials add anthropic' or set environment variables."
        );
    } else {
        info!(
